...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from8, paymaster, paymasterInput } = transaction;
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce, to, from: from8, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce, to, from: from8, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from8),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from8, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from8,
    to ?? "0x",
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "keychain") {
    const keychain = envelope;
    return concat(serializedKeychainType, keychain.userAddress, serialize(keychain.inner), options.magic ? magicBytes : "0x");
  }
  throw new CoercionError({ envelope });
}
...
function verify4(signature, parameters) {
  ...
  if (!address)
    return false;
  const envelope = from3(signature);
  if (envelope.type === "secp256k1") {
    if (!address)
      return false;
    return verify({
...
function from4(authorization, options = {}) {
...
function from5(authorization, options = {}) {
  ...
  if (options.signature)
    return {
      ...authorization,
      signature: from3(options.signature)
    };
  return authorization;
}
...
function from7(envelope, options = {}) {
  ...
  return {
    ...envelope_,
    ...signature ? { signature: from3(signature) } : {},
...
function serialize2(envelope, options = {}) {
  ...
  const serialized = [
    ...
    ...signature ? [serialize(from3(signature))] : []
  ];
...
//# sourceMappingURL=chunk-5HG7CHKZ.js.map
